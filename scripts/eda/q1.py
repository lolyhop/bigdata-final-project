"""EDA query 1: run q1.hql against Hive and export results to output/q1.csv."""
...
def main() -> int:
    ...
    conn = None
    try:
        raw = hive_utils.read_hql("eda/q1.hql")
        conn = hive_utils.open_connection(database=settings.HIVE_DATABASE)
        hive_utils.execute_hql(conn, raw)
        hive_utils.fetch_to_csv(conn, "q1_results", _OUTPUT_DIR / "q1.csv")
        LOGGER.info("q1_results table populated in %s.", settings.HIVE_DATABASE)
