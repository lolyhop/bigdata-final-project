"""EDA query 2: run q2.hql against Hive and export results to output/q2.csv."""
...
def main() -> int:
    ...
    conn = None
    try:
        raw = hive_utils.read_hql("eda/q2.hql")
        conn = hive_utils.open_connection(database=settings.HIVE_DATABASE)
        hive_utils.execute_hql(conn, raw)
        hive_utils.fetch_to_csv(conn, "q2_results", _OUTPUT_DIR / "q2.csv")
        LOGGER.info("q2_results table populated in %s.", settings.HIVE_DATABASE)
