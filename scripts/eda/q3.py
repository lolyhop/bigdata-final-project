"""EDA query 3: run q3.hql against Hive and export results to output/q3.csv."""
...
def main() -> int:
    ...
    conn = None
    try:
        raw = hive_utils.read_hql("eda/q3.hql")
        conn = hive_utils.open_connection(database=settings.HIVE_DATABASE)
        hive_utils.execute_hql(conn, raw)
        hive_utils.fetch_to_csv(conn, "q3_results", _OUTPUT_DIR / "q3.csv")
        LOGGER.info("q3_results table populated in %s.", settings.HIVE_DATABASE)
