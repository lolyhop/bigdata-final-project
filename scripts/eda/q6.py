"""EDA query 6: run q6.hql against Hive and export results to output/q6.csv."""
...
def main() -> int:
    ...
    conn = None
    try:
        raw = hive_utils.read_hql("eda/q6.hql")
        conn = hive_utils.open_connection(database=settings.HIVE_DATABASE)
        hive_utils.execute_hql(conn, raw)
        hive_utils.fetch_to_csv(conn, "q6_results", _OUTPUT_DIR / "q6.csv")
        LOGGER.info("q6_results table populated in %s.", settings.HIVE_DATABASE)
