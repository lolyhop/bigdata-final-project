...
| Relational DB         | Citus (`team25_projectdb`)     |
...
| Stage 1    | `stage1.sh`     | `dataset_filtered.csv`  | Citus bulk load via `COPY`; Sqoop export with 4 mappers                                       | Citus `loans` table; HDFS `/user/team25/loans/*.parquet`               |
...
The script `stage1.sh` transfers the Citus table to HDFS using **Apache Sqoop**. Before each import, the target directory is cleared with `hdfs dfs -rm -rf` to ensure the pipeline can be re-run cleanly. Sqoop reads the table with 4 parallel mappers, splitting the work by the `id` column.
...
The `loans` table was loaded into the `team25_projectdb` Citus database with 2,260,701 rows. Sqoop exported the table to HDFS at `/user/team25/loans` as a Snappy-compressed Parquet file. A copy of the Parquet file is archived in the `output/` directory of the repository.
...
This tab covers the structural characteristics of the `loans` Citus table and the cleaning steps applied before ingestion:
...
3. **Sqoop DATE serialisation** — Sqoop serialises Citus DATE columns as BIGINT millisecond timestamps when writing Parquet files. This required an explicit `CAST(TO_DATE(FROM_UNIXTIME(CAST(col / 1000 AS BIGINT))) AS DATE)` conversion step when creating the Hive staging table, which was a non-obvious source of type mismatch errors during development.
...
| Citus ingestion      | Design the `loans` DDL with appropriate column types and five CHECK constraints, bulk-load the filtered CSV via `COPY FROM STDIN`, run `ANALYZE`                                                                                                                               | 50%            | 0%                 | 0%           | 50%                   | Citus `loans` table, `sql/create_table.sql`, `scripts/load_postgres.py`                                                                           | 1                |
| HDFS transfer             | Configure Apache Sqoop to export the Citus table to HDFS with 4 parallel mappers, Parquet format, and Snappy compression                                                                                                                                                  | 50%            | 0%                 | 0%           | 50%                   | `/user/team25/loans/*.parquet`, `scripts/stage1.sh`                                                                                                    | 1.5              |
...
| Dashboard development     | Set up Apache Superset, register Hive and Citus datasets, build three-tab dashboard (Data Description, Data Insights, ML Modeling) with 15+ charts including bar charts, line chart, heatmap, bubble chart, and confusion matrix heatmaps                                 | 35%            | 35%                | 30%          | 0%                    | Superset dashboard, `scripts/load_ml_hive_tables.py`, `sql/ml_results.hql`                                                                             | 3                |
